...
| `validate` | `validateIndex`, `validateManifest`, `validateScreen`, `validateBundle` (+ `rebaseBundle`, `LIMITS`, the id/version regexes). Every limit in the spec is enforced. Icon names are checked against `ICON_NAMES` unless you pass your own `icons` list, or `[]` to skip. Pass `screen` and a widget that can never intersect the panel is an error |
