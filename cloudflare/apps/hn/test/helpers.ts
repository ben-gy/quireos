...
export const ORIGIN = "https://quireos-app-hn.example.workers.dev";
...
export function check(screen: Screen, label = screen.id): { widgets: number; bytes: number } {
  const r = validateScreen(screen, { manifest, origin: ORIGIN, icons });
